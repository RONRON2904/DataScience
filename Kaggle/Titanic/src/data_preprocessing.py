...
def to_npy_male_female(csv_file):
    ...
    for i in tqdm(range(df.shape[0])):
        ...
        if sex == 1:
            nb_male += 1
            X_male = np.append(X_male, [pclass, name, age, sibsp, parch, fare, cabin, embarked])
            y_male = np.append(y_male, survived)
        else:
            nb_female += 1
            X_female = np.append(X_female, [pclass, name, age, sibsp, parch, fare, cabin, embarked])
            y_female = np.append(y_female, survived)

    X_male = X_male.reshape(nb_male, 8)
    #X_male = normalize(X_male, axis=0)  Normalizing data decreases the accuracy on validation data
    X_female = X_female.reshape(nb_female, 8)
