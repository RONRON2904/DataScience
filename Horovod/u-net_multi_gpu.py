...
#    x_train, y_train = crop_impact_area(crop_size=img_rows)
#    x_train = x_train.reshape((x_train.shape[1], 1, img_rows, img_cols))
#    y_train = y_train.reshape((y_train.shape[1], 1, img_rows, img_rows))
#    np.save('x_train.h5', x_train)
...
def get_fp_arrays(x_train, y_train, inds):
    ...
    x = x.reshape(inds.shape[0], x_train.shape[1], x_train.shape[2], x_train.shape[3])
    y = y.reshape(inds.shape[0], y_train.shape[1], y_train.shape[2], y_train.shape[3])
    return x, y
...
if __name__ == '__main__':
    ...
    callbacks = [
        ...
        EarlyStoppingByLossVal(monitor='val_loss', value=0.6, verbose=1),
        checkpointer,
        reduce_lr,
    ]

    model.fit(x=x_train, y=y_train, batch_size=32, epochs=nb_epoch, verbose=1, validation_data=(x_val, y_val), callbacks=callbacks, shuffle=True)
    stop = time.time()
    print('Total running time(min) : ',(stop-start)/60.)
