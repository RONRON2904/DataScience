...
def custom_scorer(y_true, y_pred, odds, start):
    ...
    for i in range(y_pred.shape[0]):
       # print(odds[i])
        if y_pred[i] > 0.:
            if ((1/y_pred[i] < odds[i]) & (odds[i] > 5.)) | (y_pred[i] > 0.7):
            #if ((1/y_pred[i] < odds[i]) & (odds[i] > 4)):
    ...
    plt.plot(x, y)
    plt.show()
    roi = 100*(score-bk2)/tot
    print(roi, tot, cpt, nb_win, nb_loose)
    return score, roi
...
for i in range(100):
    ...
    y = df.apply(lambda x: 1 if (x['res'] == 1) else 0, axis=1).values.reshape(-1, 1)
    odds = df['oddH'].values.reshape(-1, 1)
    ...
    xgbc = XGBC(n_estimators=2000, learning_rate=0.001, max_depth=4, objective='binary:logistic')
    #xgbc = XGBC(n_estimators=3000, learning_rate=0.001, max_depth=8, objective='binary:logistic')
    ...
    start, roi = custom_scorer(y_test, y_pred, odds_test, start)
    print(start)
    if roi > 0.:
        w += 1
    else:
