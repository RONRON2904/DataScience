...
def custom_scorer(y_true, y_pred, odds, start):
    ...
    for i in range(y_pred.shape[0]):
       # print(odds[i])
        if y_pred[i] > 0.:
            if ((1/y_pred[i] < odds[i]) & (odds[i] > 5.)) | (y_pred[i] > 0.6):
    ...
    plt.plot(x, y)
    plt.show()
    print(100*(score-bk)/tot, tot, cpt, nb_win, nb_loose)
    return score
...
for i in range(15):
    ...
    xgbc = XGBC(n_estimators=5000, learning_rate=0.001, max_depth=10, objective='binary:logistic', n_jobs=-1)
